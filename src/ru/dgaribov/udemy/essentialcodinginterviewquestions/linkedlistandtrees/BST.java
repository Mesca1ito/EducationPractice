...
public class BST {
    ...
    static boolean isBST(TreeNode node, int leftBorder, int rightBorder) {
        if (node == null) return false;
        if (hasNoChildren(node)) {
            return leftBorder <= node.value && rightBorder > node.value;
        }

        return (node.left == null || isBST(node.left, leftBorder, node.value))
                && (node.right == null || isBST(node.right, node.value, rightBorder));
    }

    private static boolean hasNoChildren(TreeNode node) {
